...
任务：
1. 对照用户原始需求审查草稿，确保主题、体裁、语气、格式、长度等约束都被满足。
2. 如果草稿存在重复段落、重复观点、未满足长度要求、内容不完整、标题不必要或格式不适合写入，请直接修订为最终可写入版本。
3. 如果草稿已经合格，原样返回草稿正文。
4. 必须输出一篇完整、连贯、可直接写入文档的正文。
5. 不要解释审查过程，不要输出“已修改”“审查通过”等状态说明，不要添加代码围栏、引号、标签或额外前后缀。
6. 不要输出任何 emoji 表情符号或颜文字。`,
...
2. 优先服从用户明确提出的主题、体裁、结构、长度、语气和格式要求；用户要求短文时，不要额外扩写标题、小节或说明
3. 输出必须完整、连贯，可直接写入文档
4. 只有当用户要求或内容确有需要时才使用标题、列表或 Markdown 表格
5. 不要添加额外解释、标签、引号或前缀
6. 不要输出任何 emoji 表情符号或颜文字`,
