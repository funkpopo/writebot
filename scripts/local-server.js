...
function queryServiceState() {
  return lifecycle.queryServiceState();
}

function stopService(baseDir, exePath) {
  lifecycle.stopService(baseDir, exePath);
}

function startService(baseDir, exePath) {
  lifecycle.startService(baseDir, exePath);
}

async function waitForServiceState(expected, timeoutMs) {
  return lifecycle.waitForServiceState(expected, timeoutMs);
}

function isProcessRunningByPath(exePath) {
  return lifecycle.isProcessRunningByPath(exePath);
}

function stopProcessByPath(exePath) {
  lifecycle.stopProcessByPath(exePath);
}

async function waitForProcessExit(exePath, timeoutMs) {
  return lifecycle.waitForProcessExit(exePath, timeoutMs);
}
...
const proxyModule = require('./server/proxy');
const lifecycleModule = require('./server/lifecycle');
...
function getAddressBlockReason(address) {
  return proxyModule.getAddressBlockReason(address);
}

async function resolveTargetAddresses(hostname) {
  return proxyModule.resolveTargetAddresses(hostname);
}

async function assertAllowedProxyTarget(parsedTarget) {
  return proxyModule.assertAllowedProxyTarget(parsedTarget);
}

async function createOutboundAgent(parsedTarget, proxySettings) {
  return proxyModule.createOutboundAgent(parsedTarget, proxySettings);
}

function collectRequestBody(req) {
  return proxyModule.collectRequestBody(req);
}
...
function checkWordProcess(callback) {
  lifecycle.checkWordProcess(callback);
}
...
const lifecycle = lifecycleModule.createLifecycle({
  ...
  logsDir: path.join(isPkg ? path.dirname(process.execPath) : path.resolve(__dirname, '..'), 'logs'),
  tasklistExe: TASKLIST_EXE,
});
...
const handleApiProxy = proxyModule.createProxyHandler({
  sendJson,
...
function cleanupOldLogs(maxAgeDays = 7) {
  lifecycle.cleanupOldLogs(maxAgeDays);
}
